...
namespace Yandex.Detector
{
  ...
  public sealed class MobileDetectorTests
  {
    ...
    {
      Assert.Throws<ArgumentNullException>(() => new MobileDetector().Detect((IDictionary<string, object>) null));

      try
      {
        new MobileDetector().Detect(new Dictionary<string, object>());
      ...
      try
      {
        new MobileDetector().Detect(new Dictionary<string, object> { { "wap-profile", "invalid" } });
        throw new InvalidOperationException();
      }
      catch (DetectorException exception)
      {
      ...
      try
      {
        new MobileDetector().Detect(new Dictionary<string, object> { { "user-agent", "invalid" } });
        throw new InvalidOperationException();
      }
      catch (DetectorException exception)
      {
      ...
      var device = new MobileDetector().Detect(new Dictionary<string, object> { { "wap-profile", "http://www-ccpp-mpd.alcatel.com/files/ALCATEL-CTH3_MMS10_1.0.rdf" } });
      ...
      device = new MobileDetector().Detect(new Dictionary<string, object> { { "user-agent", "Alcatel-CTH3/1.0 UP.Browser/6.2.ALCATEL MMP/1.0" } });
      Assert.Equal("Java MIDP2 (small)", device.Description);
      Assert.Equal("midp2ss", device.DeviceClass);
      Assert.True(device.Java.Camera);
      Assert.Equal(string.Empty, device.Java.Certificate);
